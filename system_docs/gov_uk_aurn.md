...
## Register download
- Use `scripts/uk_air_sos/uk_air_sos_site_register.py` to fetch the CSV from the search results page.
- The CSV includes network membership for each site; filter to AURN for derived reports.
- Use `--dropbox-upload` to store the CSV under `{UK_AQ_DROPBOX_ROOT}/network_info/uk_air_sos`.
- Output files are timestamped (e.g., `uk_air_sos_site_register_YYYYMMDDTHHMMSSZ.csv`).
