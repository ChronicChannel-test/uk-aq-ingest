...
- Monitor:
  ...
  - For connectors configured with `HISTORY_WRITE_MODE=pubsub_only` (for example OpenAQ Cloud Run), monitor Pub/Sub backlog + `workers/uk_aq_history_pubsub_cloud_run` hourly drain summaries instead of main DB outbox depth.
