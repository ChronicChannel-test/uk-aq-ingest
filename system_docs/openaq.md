...
- Uses per-station scheduling in `uk_aq_raw.openaq_station_checkpoints` (next_due_at, last_observed_at, sample arrays).
  ...
  default interval (until 10 interval samples exist) and a 15-minute default lag (until 10 lag
  samples exist). If no observations are returned and `next_due_at` is null, it is set to
  `now() + 15 minutes`.
