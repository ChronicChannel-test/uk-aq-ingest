...
- Skips dispatch if any connector is in-flight (latest `uk_aq_ingest_runs` row has null `run_ended_at` within 10 minutes), and marks `last_run_start` before dispatch.
  ...
  - Dispatch attempts use a conditional claim on `connectors` to prevent duplicate starts across overlapping cron invocations.
...
- Notes:
  ...
  - Tracks per-station scheduling in `uk_aq_raw.openaq_station_checkpoints` (next due, last observed, sample arrays, last polled); when fewer than 10 interval/lag samples exist, `next_due_at` is set to `now() + 5 minutes`. Otherwise it uses the minimum interval + minimum lag from samples. If no observations are returned and `next_due_at` is null, it is set to `now() + 5 minutes`.
