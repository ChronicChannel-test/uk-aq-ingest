...
2) **Resolve PCON GeoJSON from Dropbox**
   ...
   - If a direct GeoJSON path is provided, uses it. Otherwise lists version folders under the base (supports year-only or month/year names like `July_2024`), picks the requested `PCON_VERSION` when possible, then finds the GeoJSON under that folder (recursively, e.g., `BFC/Data/...geojson`), preferring the latest month/year when multiple are present.
