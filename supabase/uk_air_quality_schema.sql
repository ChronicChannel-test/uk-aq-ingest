...
begin
  if to_regclass('public.connectors') is not null
     and exists (
       ...
       where table_schema = 'public'
         and table_name = 'connectors'
         and column_name = 'display_name_template'
     )
     and not exists (
       select 1
       from information_schema.columns
       where table_schema = 'public'
         and table_name = 'connectors'
         and column_name = 'station_display_name_template'
     ) then
    alter table public.connectors rename column display_name_template to station_display_name_template;
  end if;
end $$;

alter table if exists connectors add column if not exists connector_code text;
alter table if exists connectors add column if not exists display_name text;
alter table if exists connectors add column if not exists station_display_name_template text;
...
update connectors
set station_display_name_template = '{station_name}'
where service_url = 'https://uk-air.defra.gov.uk/sos-ukair/api/v1'
  or label = 'UK-AIR-SOS';
...
do $$
begin
  if to_regclass('public.uk_aq_network_pollutants') is not null
     and to_regclass('public.uk_air_sos_network_pollutants') is null then
    alter table public.uk_aq_network_pollutants rename to uk_air_sos_network_pollutants;
  end if;
end $$;
...
begin
  if exists (
    ...
    alter table station_network_memberships
      drop constraint station_network_memberships_network_code_fkey;
  end if;
end $$;
