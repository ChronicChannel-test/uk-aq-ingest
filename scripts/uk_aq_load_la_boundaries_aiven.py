#!/usr/bin/env python3
"""
Load LA boundaries into Aiven PostGIS.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Load LA boundaries into Aiven.")
    ...
    parser.add_argument(
        "--skip-if-exists",
        action="store_true",
        help="Skip boundary uploads if the target LA version already exists.",
    )
    ...
    try:
        with conn.cursor() as cursor:
            if args.skip_if_exists:
                cursor.execute(
                    "select 1 from la_boundaries where la_version = %s limit 1",
                    (args.la_version,),
                )
                if cursor.fetchone():
                    print(f"Boundaries already exist for {args.la_version}; skipping upload.")
                    return 0
        ...
        query = (
            "insert into la_boundaries (la_code, la_name, la_version, geometry) "
            "values %s "
